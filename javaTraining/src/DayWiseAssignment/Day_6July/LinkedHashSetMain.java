/**
 * LinkedHashSetOperations.java
 *
 * Complete demonstration of every public method available on java.util.LinkedHashSet,
 * plus the Collections utility methods (min, max, sort, etc.) commonly used with it.
 *
 * IMPORTANT NOTE ON LinkedHashSet:
 * - LinkedHashSet MAINTAINS insertion order (unlike HashSet which has no guaranteed order).
 * - LinkedHashSet does NOT allow duplicate elements - adding a duplicate is silently ignored.
 * - LinkedHashSet has NO index-based methods: no get(int), set(int, E), add(int, E),
...
public class LinkedHashSetMain {
	...
	public static void main(String[] args) {
        ...
        LinkedHashSet<String> setCap = new LinkedHashSet<>(32);                      // initial capacity
        LinkedHashSet<String> setCapLoad = new LinkedHashSet<>(32, 0.5f);            // capacity + load factor
        LinkedHashSet<String> setFromCollection = new LinkedHashSet<>(Arrays.asList("X", "Y", "Z")); // from Collection
        ...
        System.out.println("(Note: order shown IS exactly insertion order - LinkedHashSet guarantees it)");
        ...
        // NOTE: There is no indexOf()/lastIndexOf() - LinkedHashSet has no positions/index.
        ...
        LinkedHashSet<String> toRemoveAll = new LinkedHashSet<>(Arrays.asList("Banana"));
        set.removeAll(toRemoveAll);                   // removeAll(Collection)
        System.out.println("After removeAll([\"Banana\"]): " + set);

        LinkedHashSet<String> retainDemo = new LinkedHashSet<>(setFromCollection);
        ...
        // NOTE: There is NO ListIterator for a LinkedHashSet (only List has that).
        ...
        LinkedHashSet<String> cloned = (LinkedHashSet<String>) set.clone();  // clone()
        System.out.println("\nclone(): " + cloned);
        ...
        LinkedHashSet<String> toClear = new LinkedHashSet<>(set);
        toClear.clear();                                // clear()
        ...
        System.out.println("\n\n========== Collections utility methods on LinkedHashSet<Integer> ==========");

        LinkedHashSet<Integer> nums = new LinkedHashSet<>(Arrays.asList(15, -4, 8, 42, -23, 16, 4));
        ...
        LinkedHashSet<Integer> absValues = new LinkedHashSet<>();
        for (int n : nums) {
        ...
        LinkedHashSet<Integer> other = new LinkedHashSet<>(Arrays.asList(1, 2, 3));
